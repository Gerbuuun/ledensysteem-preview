...
export const createStore = <T extends { id?: string }>(params: {
  ...
  return defineStore(params.id, {
    ...
    actions: {
      async fetchData() {
        const response = await useHttpClient().apiRequest<T[]>(`${process.env.API_URL}/${params.base}`);
        if (response) {
      ...
      async loadSelected() {
        ...
        const response = await useHttpClient().apiRequest<T>(`${process.env.API_URL}/${params.single}/${this.selectedId}`);
        if (response) {
      ...
      async create(data: T): Promise<boolean> {
        ...
        const response = await useHttpClient().apiRequest<T>(`${process.env.API_URL}/params.base`, { data: data, method: 'POST' });
        if (response) {
          this.entities.set(response.id, response);
      ...
      async update(data: T): Promise<boolean> {
        ...
        const response = await useHttpClient().apiRequest<T>(`${process.env.API_URL}/${params.single}/${this.selectedId}`, { data: data,  method: 'PATCH' });
        if (response) {
      ...
      async delete(): Promise<boolean> {
        const response = await useHttpClient().apiRequest<T>(`${process.env.API_URL}/${params.single}/${this.selectedId}`, { method: 'DELETE' });
        if (response) {
          this.selectedId = null;
